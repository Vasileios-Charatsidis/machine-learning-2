...
, (M-Step): We maximise over $\theta_{t+1}$. So $B$ in eq.(13) must be bigger
...
iterations. Hence, $h(\theta_{t}) \leq h(\theta_{t+1})$
...
= \mathbb{E}_{Z|X,\theta^{old}}[\text{ln } p(X,Z|\theta)]$
...
drawn from distribution q and weighed correspondingly:
$x_i \approx \tilde{q}$, $w_i = \frac{\tilde{p}(x_i)}{\tilde{q}(x_i)}$
Works bad in high dimensions as it is hard to find an x where both q(x) and p(x)
are high in the high-dimensional space. If p and q overlap just a little, this
will give a bad estimate.
