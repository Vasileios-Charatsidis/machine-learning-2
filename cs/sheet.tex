...
For the first node:\\
$p(z_1|\pi) = \prod_{k=1}^K \pi_k^{z_{1,k}}$ with $\sum_{k=1}^K\pi_k=1$ \\
$p(z_n|z_{n-1},A) = \prod_{k=1}^K \prod_{j=1}^K A_{jk}^{z_{n-1},jz_{nk}}$
	with $\sum_{k=1}^K A_{jk}=1$ \\
...
\textbf{E-step:} to be solved with sum-product \\
$Q(\theta|\theta^{old}) = \sum_Z p(Z|X,\theta^{old})\ln p(X,Z|\theta)$ \\
\textbf{M-step:} normal maximization with Lagrangians \\
...
\textit{Gaussian emission}: \\
$\mu_k = \frac{\sum_{n=1}^N\gamma(z_{nk})x_n}{\sum_{n=1}^N\gamma(z_{nk})}$ \quad
$\Sigma_k =
	\frac{\sum_{n=1}^N\gamma(z_{nk})(x_n-\mu_k)(x_n-\mu_k)^\top}{\sum_{n=1}^N\gamma(z_{nk})}$ \\
\textit{Discrete multinomial emission}:
...
Finally we have the result for the E step: \\
$\gamma(z_n) = \frac{p(z_n,X)}{p(X)} = \frac{\alpha(z_n)\beta(z_n)}{p(X)}$ \\
...
\begin{enumerate}
	\item Ignoring observations: \\
		$p(y | \mathrm{do}(x), w, z) = p(y | \mathrm{do}(x), w)$
		if $(y \ci z | x,w)_{\mathcal{G}_{\overline x}}$
	\item Action / observ. exchange: \\
		$p(y | \mathrm{do}(x), \mathrm{do}(z), w) = p(y | \mathrm{do}(x), z, w)$
		if $(y \ci z | x, w)_{\mathcal{G}_{\overline{x}, \underline{z}}}$
	\item Ignoring actions: \\
		$p(y | \mathrm{do}(x), \mathrm{do}(z), w) = p(y | \mathrm{do}(x), w)$
		if $(y \ci z | x, w)_{\mathcal{G}_{
			\overline{x}, z \setminus \text{An}_{\mathcal{G}_{\overline{x}}}(w)}}$
\end{enumerate}
